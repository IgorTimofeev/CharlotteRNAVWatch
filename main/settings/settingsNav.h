	...
	class SettingsNav : public NVSSerializable {
		...
		protected:
			...
			void onRead(const NVSStream& stream) override {
				...
				waypointIndex = stream.getUint16(_waypointIndex, 0);
				course = stream.getUint16(_course, 0);
			}

			void onWrite(const NVSStream& stream) override {
				stream.setUint16(_waypointIndex, waypointIndex);
				stream.setUint16(_course, course);
			}

		private:
			constexpr static auto _namespace = "nav";
			constexpr static auto _waypointIndex = "wi";
			constexpr static auto _course = "cr";
